...
env.roledefs = {
    "web": [
        "save-the-unicorns@scorch.devsoc.org",
        "save-the-unicorns@smaug.devsoc.org",
    ],
    "demo": [
        "save-the-unicorns@trogdor.devsoc.org",
    ],
    "cron": [
        "save-the-unicorns@scorch.devsoc.org",
    ],
...
env.home = env.get("home", "/var/www/save-the-unicorns")
env.virtualenv = env.get("virtualenv", "/var/envs/save-the-unicorns")
env.appname = env.get("appname", "save-the-unicorns")
env.repo = env.get("repo", "save-the-unicorns")
env.media = env.get("media", "save-the-unicorns")
env.media_bucket = env.get("media_bucket", "contentfiles-media-eu-west-1")
env.database = env.get("database", "save-the-unicorns_django")
...
def sentry_release():
    ...
    with cd(env.home):
        ...
        run(
            "sentry-cli releases deploys {version} new --env $SENTRY_ENVIRONMENT".format(
                version=version
            )
        )
